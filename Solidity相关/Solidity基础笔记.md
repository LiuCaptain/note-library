...
33. #### 多重合约继承中的 C3 线性化
    ...
    - C3 线性序列化算法描述：
      1. 从当前合约开始
      2. **从右到左**找到它的一个父合约
      3. 判断该父合约下的子合约是否都已经被遍历
      4. 如果是，则该合约入列并继续**从右到左**找到该入列合约的父合约，重新到第 3 步开始执行
      5. 如果不是，则回退到上一个子合约，**从右到左**找到该子合约的兄长合约
      6. 如果该兄长合约**也是**这个子合约的父合约，则再次回退到上一个子合约，**从右到左**找到上一个子合约的兄长合约，重新到第 2 步开始执行
      7. 如果该兄长合约**不是**这个子合约的父合约，判断该兄长合约的子合约是否都应被遍历，重新到第 4 步开始执行
    - 多重合约继承中，`constructor` 的执行顺序是按照 C3 线性化输出的顺序来执行的
    - 多重合约继承中，`super` 是 C3 线性化输出的序列中当前合约的前驱合约
    - 多重合约继承中，状态变量的储存布局（`storage layout`）顺序是参考 C3 线性化后的继承顺序来决定的，但最终遵循“最基础父合约 -> 更派生合约”的顺序
